...
$('#formsizee').submit(function() {
   if(Validate(0)==idinpute.length){
    $.ajax({
            type: "POST",
            url: "../../controller/csize.php?updateData=update", 
            data: $("#formsizee").serialize(),
            success: function(resp) {
                   if(resp==1){
                    M.toast({html: "¡Se ha modificado el tamaño exitosamente!", classes: 'rounded  green'});
                    $('.modal').modal('close');
                    getData();
                    cleanform();
                    cleanbox();
...
var idinput = ['txtnumber','txtname'];
var idinputerror= ['txtnumbererror','txtnameerror'];
var idinpute = ['txtnumbere','txtnamee'];
var idinputerrore= ['txtnumbererrore','txtnameerrore'];
...
var FillBoxes =(id,number,name) =>{
    $("#sizeid").val(id);
    $("#txtnumbere").val(number);
    $("#txtnumbere").focus();
    $("#txtnamee").val(name); 
}
...
var StateChange = (id,estado) =>{
    ...
    var paren = id
          var dataString = 'id='+id+"&state="+estado;
           $.ajax({
            type: "POST",
            url: "../../controller/csize.php?updateData=statechange",
            data: dataString,
            success: function(resp) {            
            //alert(resp);
                    if (resp=="1") {
...
var getData = ()=> {
    ...
    "ajax": {
      url: "../../controller/csize.php?btngetData=getData",//hasta para consultar tenemos un boton imaginario en el controlador  => ($page = isset($_GET['btnConsultar'])?$_GET['btnConsultar']:'';)
          "type": "POST"
    },
    "columns": [
      { "data": "number_size" },
