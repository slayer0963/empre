...
include_once "../cn/connection.php";
 ...
 class DAOMonitoring
 {
    ...
    {
        $c = conectar();
        $sql="select CONVERT(SUM(scd.quantity*scd.precio), DECIMAL(15,2)) as total, date_format(cart.datesold,'%d-%m-%Y') as dates from shopping_cart_details scd inner join shopping_cart cart on cart.id_shp_c=scd.id_shp_c inner join clients cli on cli.id_cl = cart.id_cl inner join assignment_details_general adg on adg.id_prices=scd.id_prices and adg.id_color=scd.id_color and adg.id_material=scd.id_mat and adg.id_size=scd.id_size inner join assignment_prices_object apo on apo.id_prices = adg.id_prices inner join assignment_probus probus on probus.id_pro=apo.id_pro inner join product pro on pro.id_pro=probus.id_pro inner join product_type pt on pt.id_tpro=pro.id_tpro inner join business bu on bu.id_bus=probus.id_bus inner join delivery del on del.id_shop_c=cart.id_shp_c where bu.id_bus=$idbus and adg.id_prices=$idprice and adg.id_color=$color and adg.id_material=$mat and adg.id_size=$size and del.status_delivery=1 group by dates";
        $c->set_charset('utf8'); 
        $res = $c->query($sql); 
        $arreglo = array();
        while($re = $res->fetch_array()){
    ...
    {
        $c = conectar();
        $sql="select CONVERT(SUM((scd.quantity*scd.precio))-SUM(scd.quantity*apo.pur_price), DECIMAL(15,2)) as ganancia, date_format(cart.datesold,'%d-%m-%Y %h:%m:%s') as dates from shopping_cart_details scd inner join shopping_cart cart on cart.id_shp_c=scd.id_shp_c inner join clients cli on cli.id_cl = cart.id_cl inner join assignment_details_general adg on adg.id_prices=scd.id_prices and adg.id_color=scd.id_color and adg.id_material=scd.id_mat and adg.id_size=scd.id_size inner join assignment_prices_object apo on apo.id_prices = adg.id_prices inner join assignment_probus probus on probus.id_pro=apo.id_pro inner join product pro on pro.id_pro=probus.id_pro inner join product_type pt on pt.id_tpro=pro.id_tpro inner join business bu on bu.id_bus=probus.id_bus inner join delivery del on del.id_shop_c=cart.id_shp_c where bu.id_bus=$idbus and adg.id_prices=$idprice and adg.id_color=$color and adg.id_material=$mat and adg.id_size=$size and del.status_delivery=1";
        $c->set_charset('utf8'); 
        $res = $c->query($sql); 
        $arreglo = array();
        while($re = $res->fetch_array()){
