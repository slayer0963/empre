 ...
 */
 class DAOUserhome
 {
    ...
    public function getProductClientbyrangehome($range)
    {
        ...
        $sql="select asdg.id_color,asdg.id_material,asdg.id_size, pro.id_pro, pro.name_pro, asdg.img, asdg.quantity, aspo.pur_price, aspo.sal_price, asdg.discount, pro.descr_pro, asdg.extraprice, bu.name_bus from assignment_details_general asdg inner join assignment_prices_object aspo on asdg.id_prices=aspo.id_prices inner join assignment_probus aspro on aspro.id_pro=aspo.id_pro inner join product pro on pro.id_pro=aspro.id_pro inner join business bu on aspro.id_bus=bu.id_bus where ((aspo.sal_price + asdg.extraprice)-((aspo.sal_price + asdg.extraprice)*asdg.discount))<=$range group by id_pro;";
        $c->set_charset('utf8');
        $res = $c->query($sql); 
        $arreglo = array();
        while($re = $res->fetch_array()){
